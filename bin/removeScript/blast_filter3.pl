...
# query或hit至少要被hsp覆盖一定比例（默认0.75）
# 相同的<query,hit>中，只保留evalue最低的(即第一个)
# 一个query对应的hits中 只保留比最低evalue那个hsp的identity少于drop_off(5%)以内的
if (@ARGV < 2)
{
  print "usage: blast_filter3.pl input output coverage identity_dropoff\n";
  exit(0);
}
...
while (<IN>) {
	my @cols = split(/\t/, $_);
	if(1.0*$cols[4]/$cols[1] >= $coverage || 1.0*$cols[4]/$cols[3] >= $coverage){#query或hit至少要被hsp覆盖一定比例
